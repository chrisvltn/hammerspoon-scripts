...
function expander()
	...
	keyWatcher = hs.eventtap.new({ hs.eventtap.event.types.keyDown }, function(ev)
		...
		if isExpanding == true then
			if keyCode == keyMap["delete"] then
				return false
			end
			
			currentOutput = currentOutput .. char
			obj.logger.d("Is expanding")
			obj.logger.df("Current expansion '%s' (%s)", currentOutput, string.len(currentOutput))
			obj.logger.df("Final expansion '%s' (%s)", finalOutput, string.len(finalOutput))
			
			if currentOutput == finalOutput then
				obj.logger.d("Finished expansion")
				currentOutput = ""
				finalOutput = ""
				isExpanding = false
			end
		...
		if keyCode == keyMap["delete"] then
			...
			if #word > 0 then
				...
				obj.logger.df("Word after deleting: %s", word)
			end
		...
		word = word .. char
		obj.logger.df("Word after appending: %s", word)
		...
		if output or
		type(obj.keywords[word]) == "function" then
